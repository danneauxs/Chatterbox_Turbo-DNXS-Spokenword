...
class TerminalLogger:
    ...
    def __init__(self, log_file="term.log", also_print=True):
        """Initializes a logger that can optionally suppress console output and log to a file.
        Args:
        log_file (str): Path to the log file. Defaults to "term.log".
        also_print (bool): Whether to print logs to the console in addition to logging them to the file. Defaults to True.
        Returns: None
        """
    ...
    def set_eta_frequency(self, every: int):
        """Sets the frequency of ETA updates.
        Args:
        every (int): The interval at which ETA should be updated.
        Returns: None
        ---
        Sets the batch size for processing.
        Args:
        size (int): The number of items per batch.
        Returns: None
        """
        with self.lock:
            try:
...
def set_eta_frequency(every: int):
    """Set the ETA frequency for logging.
    Args:
    - every: The frequency at which to update the ETA.
    ---
    Set the batch size for processing.
    Args:
    - size: The size of each batch.
    ---
    Emit a summary of the current chunk being processed.
    """
    global _terminal_logger
    if _terminal_logger is not None:
...
def set_batch_size(size: int):
    """Sets the batch size for the terminal logger.
    Args:
    size (int): The new batch size.
    Returns:
    None
    Emits a chunk summary using the terminal logger.
    Args:
    None
    Returns:
    None
    Gets the running average iterations per second from the terminal logger's summaries.
    Args:
    None
    Returns:
    float: The running average it/s.
    """
    global _terminal_logger
    if _terminal_logger is not None:
...
def emit_chunk_summary():
    """Emit a chunk summary using the global terminal logger.
    Args:
    None
    Returns:
    None
    ---
    Retrieve the running average iterations per second from per-chunk summaries.
    Args:
    None
    Returns:
    The running average it/s as a float, or None if _terminal_logger is not initialized or an exception occurs.
    """
    global _terminal_logger
    if _terminal_logger is not None:
