...
CHATTERBOX_CKPT_DIR = os.environ.get("CHATTERBOX_CKPT_DIR", "")  # Let from_pretrained() auto-download chatterbox-turbo
...
ENABLE_ASR = False   # ASR validation enabled by default
ASR_WORKERS = 4                       # Parallel ASR on CPU threads
DEFAULT_ASR_MODEL = "small"           # Default Whisper model for ASR validation
DEFAULT_ASR_THRESHOLD = 0.85         # Default similarity threshold for ASR validation
ASR_WORKERS = 4                       # Parallel ASR on CPU threads
DEFAULT_ASR_MODEL = "small"           # Default Whisper model for ASR validation
DEFAULT_ASR_THRESHOLD = 0.85         # Default similarity threshold for ASR validation
...
SILENCE_PERIOD = 500
SILENCE_QUESTION_MARK = 600
SILENCE_EXCLAMATION = 300
...
ENABLE_PUNCTUATION_PAUSES = False  # Enable inline pause tag insertion

# PUNCTUATION_PAUSE_MAPPING: Map punctuation to config constant NAMES (not values)
...
ENABLE_REGENERATION_LOOP = False      # Enable automatic chunk regeneration on quality failure
...
ENABLE_MFCC_VALIDATION = False        # Enable MFCC-based spectral analysis
SPECTRAL_VARIANCE_LIMIT = 100.0      # Maximum spectral variance before flagging as artifact

# --- Output Validation Settings ---
ENABLE_OUTPUT_VALIDATION = False      # Enable quality control clearinghouse (runs individual checks when enabled)
...
# WARNING: FP16 is EXPERIMENTAL and may affect audio quality. Only enable if you understand the trade-offs.
# Estimated impact: ~50% memory reduction, 1.5-2x speed improvement, potential audio artifacts
# Test thoroughly with regeneration before enabling in production.
ENABLE_FP16_PRECISION = True       # Use half-precision (FP16) for all TTS models (EXPERIMENTAL)
...
# ============================================================================
REGEN_TEMPERATURE_ADJUSTMENT = 0.1     # Per attempt temperature reduction
REGEN_EXAGGERATION_ADJUSTMENT = 0.15   # Per attempt exaggeration reduction
REGEN_CFG_ADJUSTMENT = 0.1             # Per attempt CFG weight reduction
REGEN_MAX_ATTEMPTS = 4                 # Total attempts per failed chunk (original + 3 regenerations)
