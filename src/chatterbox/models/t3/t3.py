...
def _ensure_BOT_EOT(text_tokens: Tensor, hp):
    """Ensures that the input tensor contains start and stop text tokens.
    Args:
    text_tokens: Tensor representing the sequence of text tokens.
    hp: Hyperparameters object containing start_text_token and stop_text_token attributes.
    Returns:
    None
    """
...
class T3(nn.Module):
    ...
    def __init__(self, hp=None):
        """Initialize a language model configuration.
        Args:
        hp (T3Config): Hyperparameters for the model. If None, uses default English-only configuration.
        Returns:
        None
        """
        if hp is None:
    ...
    @property
    def device(self):
        """Returns the device of the speech head's weight.
        Args:
        t3_cond (T3Cond): The conditioning data for T3.
        Returns:
        torch.Tensor: The embedded conditioning data.
        """
    ...
    def prepare_input_embeds(
        ...
        *,
        ...
        """Prepare input embeddings for a model, combining conditioning, text, and speech tokens.
        Args:
        t3_cond: T3Cond object containing conditioning information.
        text_tokens: torch.LongTensor of text tokens.
        speech_tokens: torch.LongTensor of speech tokens.
        cfg_weight: float weight for conditional generation, default 0.0.
        Returns:
        A tuple of embeddings (cond_emb, text_emb, speech_emb).
        """
    ...
    def forward(
        ...
        *,
        ...
        """Forward pass for a model, processing textual and speech tokens.
        Args:
        t3_cond (T3Cond): Conditional data.
        text_tokens (torch.LongTensor): Text token IDs.
        text_token_lens (torch.LongTensor): Lengths of text tokens.
        speech_tokens (torch.LongTensor): Speech token IDs.
        speech_token_lens (torch.LongTensor): Lengths of speech tokens.
    ...
    def loss(
        ...
        *,
        ...
        Args:
        t3_cond (T3Cond): Conditional input.
        text_tokens (torch.LongTensor): Text tokens.
        text_token_lens (torch.LongTensor): Lengths of text tokens.
        speech_tokens (torch.LongTensor): Speech tokens.
        speech_token_lens (torch.LongTensor): Lengths of speech tokens.
        Returns: None
        """
        "training method"
    ...
    def inference_turbo(self, t3_cond, text_tokens, temperature=0.8, top_k=1000, top_p=0.95, repetition_penalty=1.2,
                        max_gen_len=1000):
        """Runs inference using a Turbo model with specified parameters.
        Args:
        t3_cond (Tensor): Conditional tensor for the model.
        text_tokens (Tensor): Input text tokens.
        temperature (float, optional): Sampling temperature. Default is 0.8.
        top_k (int, optional): Top-k filtering parameter. Default is 1000.
        top_p (float, optional): Nucleus sampling probability. Default is 0.95.
        repetition_penalty (float, optional): Repetition penalty factor. Default is 1.2.
        max_gen_len (int, optional): Maximum generation length. Default is 1000.
        Returns:
        Tensor: Generated speech tokens.
        """
