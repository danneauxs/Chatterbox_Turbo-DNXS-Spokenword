...
def drop_invalid_tokens(x):
    """Drop tokens with values greater than or equal to SPEECH_VOCAB_SIZE.
    Args:
    x: A tensor containing the tokens to be filtered.
    Returns:
    A tensor with only the valid tokens.
    """
...
def get_resampler(src_sr, dst_sr, device):
    ...
    Returns:
    ta.transforms.Resample: Resampler object configured with specified parameters and moved to the given device.
    ```
    """
...
class S3Token2Mel(torch.nn.Module):
    ...
    def __init__(self, meanflow=False):
        """Initializes a neural network component with optional meanflow support. Args: meanflow (bool): Whether to enable meanflow in the model. Returns: None"""
        super().__init__()
    ...
    @property
    def device(self):
        """Returns the device of the first parameter in the tokenizer.
        Args:
        ref_wav (torch.Tensor): The reference waveform.
        ref_sr (int): The sample rate of the reference waveform.
        device (str, optional): Device to use, defaults to "auto".
        ref_fade_out (bool, optional): Whether to fade out the reference waveform, defaults to True.
        """
    ...
    @property
    def dtype(self):
        """Returns the data type of the first parameter in the flow.
        Args:
        self: The instance calling this method.
        Returns:
        torch.dtype: Data type of the first parameter in the flow.
        ---
        Embeds a reference waveform into the model.
        Args:
        ref_wav (torch.Tensor): Reference waveform.
        ref_sr (int): Sample rate of the reference waveform.
        device (str, optional): Device to run operations on ("auto" for automatic). Defaults to "auto".
        ref_fade_out (bool, optional): Whether to fade out the reference waveform. Defaults to True.
        Returns:
        None
        """
    ...
    def embed_ref(
        ...
        """Embeds a reference audio waveform into the model.
        Args:
        ref_wav (torch.Tensor): Reference audio waveform.
        ref_sr (int): Sample rate of the reference audio.
        device (str, optional): Device to use for computation ("auto" uses the default device). Defaults to "auto".
        ref_fade_out (bool, optional): Whether to apply a fade-out effect to the reference audio. Defaults to True.
        Returns:
        torch.Tensor: Embedded representation of the reference audio.
        """
...
class S3Token2Wav(S3Token2Mel):
    ...
    def __init__(self, meanflow=False):
        """Initializes the HiFTGenerator class with optional meanflow parameter. Sets up an instance of ConvRNNF0Predictor and initializes the mel-to-wav converter with specified parameters. Trims and fades audio to reduce artifacts. Args: meanflow (bool): Whether to use mean flow. Returns: None"""
    ...
    def flow_inference(
        ...
        """Performs inference using flow-based models for speech processing.
        Args:
        speech_tokens (list): List of speech tokens.
        ref_wav (torch.Tensor, optional): Locally-computed reference waveform embedding.
        ref_sr (int, optional): Sampling rate of the reference waveform.
        ref_dict (dict, optional): Pre-computed reference embedding dictionary.
        n_cfm_timesteps (int, optional): Number of conditioning timesteps.
        finalize (bool, optional): Flag to indicate finalization.
        speech_token_lens (list, optional): Lengths of speech tokens.
        Returns:
    ...
    def hift_inference(self, speech_feat, cache_source: torch.Tensor = None):
        """```python
        Performs inference using speech features and optional caching.
        Args:
        speech_feat (torch.Tensor): The input speech features.
        cache_source (torch.Tensor, optional): Cached source tensor for inference.
        Returns:
    ...
    def inference(
        ...
        """Performs inference on speech tokens using reference audio or embeddings.
        Args:
        speech_tokens (List[str]): List of speech tokens.
        ref_wav (Optional[torch.Tensor]): Locally-computed reference waveform embedding.
        ref_sr (Optional[int]): Reference audio sampling rate.
        ref_dict (Optional[dict]): Pre-computed reference embedding dictionary.
        drop_invalid_tokens (bool, optional): Whether to drop special tokens during inference. Defaults to True.
        n_cfm_timesteps (Optional[int]): Number of confidence timesteps.
        speech_token_lens (Optional[List[int]]): Lengths of speech tokens.
        Returns:
        List[str]: Inferred speech tokens.
        """
