...
def cast_all(*args, dtype):
    """Casts all input tensors to a specified data type if they are floating-point types.
    Args:
    *args: Input tensors.
    dtype: Target data type for casting.
    Returns:
    List of tensors cast to the specified data type.
    """
    return [a if (not a.dtype.is_floating_point) or a.dtype == dtype else a.to(dtype) for a in args]


class ConditionalCFM(BASECFM):
    """ConditionalCFM class extends BASECFM and is designed for conditional computations, incorporating speaker embeddings in its architecture based on the number of speakers specified. The class initializes with parameters related to feature dimensions, CFM settings, and optional estimator module adjustments."""
    def __init__(self, in_channels, cfm_params, n_spks=1, spk_emb_dim=64, estimator: torch.nn.Module = None):
        """Initializes a model with specified parameters and an optional estimator.
        Args:
        in_channels (int): Number of input channels.
        cfm_params: Configuration parameters for the model.
        n_spks (int, optional): Number of speakers. Default is 1.
        spk_emb_dim (int, optional): Speaker embedding dimension. Default is 64.
        estimator (torch.nn.Module, optional): Estimation module. Defaults to None.
        Returns:
        None
        """
        super().__init__(
...
class CausalConditionalCFM(ConditionalCFM):
    """Causal Conditional CFM is a subclass of ConditionalCFM designed for causal conditional diffusion models, handling audio or similar sequential data with speaker embeddings.
    This class extends functionality to include optional noise manipulation during forward diffusion, allowing control over the randomness and stability of the model's predictions.
    """
    ...
    def basic_euler(self, x, t_span, mu, mask, spks, cond):
        """Performs Euler method integration for a given state `x` over time span `t_span` with parameters `mu`, `mask`, `spks`, and `cond`.
        Args:
        x (Tensor): Initial state.
        t_span (Tensor): Time span over which to integrate.
        mu (Tensor): Parameter vector.
        mask (Tensor): Mask for the state.
        spks (Tensor): Spikes tensor.
        cond (Tensor): Conditional parameters.
        Returns:
        Tensor: Integrated state after Euler method.
        """
        in_dtype = x.dtype
        x, t_span, mu, mask, spks, cond = cast_all(x, t_span, mu, mask, spks, cond, dtype=self.estimator.dtype)
