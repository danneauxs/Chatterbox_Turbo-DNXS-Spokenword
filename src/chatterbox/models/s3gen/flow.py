...
class CausalMaskedDiffWithXvec(torch.nn.Module):
    """A PyTorch module for causal masked diffusion with speaker embeddings, designed for voice synthesis tasks."""
    def __init__(self,
                 input_size: int = 512,
                 output_size: int = 80,
        ...
        """Initialize a speech-to-speech model. Args: input_size (int): size of the input feature vectors. output_size (int): size of the output feature vectors. spk_embed_dim (int): dimension of speaker embeddings. output_type (str): type of output features. vocab_size (int): number of tokens in the vocabulary. input_frame_rate (int): frame rate of the input audio. only_mask_loss (bool): whether to use masked loss during training. token_mel_ratio (int): ratio of tokens to mel frames. pre_lookahead_len (int): length of lookahead for pre-training. encoder (torch.nn.Module): custom encoder module. decoder (torch.nn.Module): custom decoder module. decoder_conf (Dict): configuration dictionary for the decoder. Returns: None"""
        super().__init__()
    ...
    def compute_loss(
        ...
        Args:
        batch (dict): A dictionary containing input tensors.
        device (torch.device): The device to which tensors are moved.
        Returns:
        Dict[str, Optional[torch.Tensor]]: A dictionary with loss values or None.
        """
    ...
    def inference(self,
                  ...
                  noised_mels=None,
                  meanflow=False):
        """Performs inference using given tokens and features.
        Args:
        token: Input tokens of shape (B, n_toks).
        token_len: Lengths of input tokens of shape (B,).
        prompt_token: Prompt tokens of shape (B, n_toks).
        prompt_token_len: Lengths of prompt tokens of shape (B,).
        prompt_feat: Prompt features.
        prompt_feat_len: Lengths of prompt features.
        embedding: Embedding tensor.
        finalize: Whether to finalize the inference.
        n_timesteps: Number of timesteps for inference. Default is 10.
        noised_mels: Noised mel-spectrograms.
        meanflow: Whether to use mean flow. Default is False.
        Returns:
