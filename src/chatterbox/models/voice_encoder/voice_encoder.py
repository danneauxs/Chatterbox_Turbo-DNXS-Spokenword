...
def get_num_wins(
    ...
    """Calculates the number of windows and the target number of frames based on given parameters.
    Args:
    n_frames (int): Total number of frames.
    step (int): Step size between windows.
    min_coverage (float): Minimum required coverage for the last window.
    hp (VoiceEncConfig): Voice encoding configuration with partial frame size.
    Returns:
    tuple: Number of windows and target number of frames.
    """
    assert n_frames > 0
...
def get_frame_step(
    ...
    """Computes the number of frames separating two partial utterances based on overlap and rate.
    Args:
    overlap (float): The overlap ratio between consecutive partial utterances.
    rate (float): The sampling rate of the audio.
    hp (VoiceEncConfig): Configuration object containing voice encoding parameters.
    Returns:
    int: The computed frame step.
    """
...
class VoiceEncoder(nn.Module):
    """Class that implements a voice encoder using an LSTM network to process mel-spectrogram inputs and produce speaker embeddings."""
    def __init__(self, hp=VoiceEncConfig()):
        """Initializes a new instance of the model with optional voice enc configuration.
        Args:
        hp (VoiceEncConfig): Voice encoding configuration object. Defaults to VoiceEncConfig().
        Returns:
        None
        """
        super().__init__()
    ...
    def device(self):
        ...
        Args:
        mels (torch.FloatTensor): A batch of unscaled mel spectrograms of shape (B, T, M).
        Returns:
        torch.device: The device where the model is located.
        """
